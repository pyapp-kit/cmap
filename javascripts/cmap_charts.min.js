...
var cvd_type=document.getElementById("cvd").querySelector('input[name="cvd_button"]:checked')?.value;for(var cmap_name in chartElems){var cmap_data=window.cmap_data[cmap_name][cvd_type]||window.cmap_data[cmap_name]["normal"];for(var i=0;i<chartElems[cmap_name].length;i++){var canv=chartElems[cmap_name][i];const chart=Chart.getChart(canv);if(chart){chart.destroy();}
if(canv.classList.contains("rgb-chart")){makeRGBChart(canv,cmap_data);}else if(canv.classList.contains("hsl-chart")){makeHSLChart(canv,cmap_data);}else if(canv.classList.contains("linearity-chart")){makeLinearityChart(canv,cmap_data);}}}}
